...
def find(driver):
    element = driver.find_element_by_xpath('/html/body/div/div/object/div')
    if element:
        return element
    else:
        return False
...
def capture():  # 캡쳐하는애
    ...
    element = WebDriverWait(driver,7).until(find)
    im1 = element.screenshot(
        'D:/testImage/' + get_today() + '/' + time.strftime('%H') + '/'
